# Architecture: Why Three Entry Points?

agent-ear could have been a single script. Instead, it's one Python package behind two bash wrappers — all assembled into three entry points by Nix at build time. This page explains why.
...
|:------------|:-------------------|:--------|
| `agent-ear` | Bash script (Nix `writeShellApplication`) | Smart dispatcher — routes based on CLI flags and TTY state |
...
| `agent-ear-interactive` | Bash script (Nix `writeShellApplication` wrapping `scripts/agent-ear-interactive.sh`) | Terminal wizard with guided mode selection and config, uses [Gum](https://github.com/charmbracelet/gum) |

> [!NOTE] 🔍 **Context for reviewer**
> These aren't three independently compiled programs. Nix builds one Python package (`agent-ear-core`) and generates two thin bash scripts around it. Think of it like a Python package installed via `pip` that comes with two shell aliases. The "three binaries" are three files in `$PATH`, but they're really one Python program with two shell-script front-ends that Nix wires together at build time — handling PATH, library paths, and runtime dependencies automatically.
...
The **dispatcher** is the glue — a bash script generated by Nix at build time. Its routing logic is trivial:
...
This separation means agents never see the TUI code, and humans never need to know the flag syntax. Both bash scripts ultimately `exec` into the same Python entry point (`agent-ear-core`), which does all the real work.

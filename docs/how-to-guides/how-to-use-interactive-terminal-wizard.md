...
1. **Pick a mode.** The wizard opens with **"What would you like to do?"**. For the fastest path, choose **🎤 Record Now**. 'Record now' uses the built-in Quick Transcript template, so you skip prompt-writing entirely. Prefer a purpose-built prompt? Choose **📋 Templates ▸** and pick one (Meeting Notes, Brain Dump, Dictation, Interview, Lecture Notes), or **📝 Custom Prompt** to write your own.

2. **Configure the basics.** The wizard asks for output format (`markdown` for Obsidian notes), transcription model (live $/min pricing is shown next to each, Flash is the default), output directory, and an optional topic for the filename. Optionally, **⚙️ Advanced Options** (GCS bucket, high-res video, validation skip) is there if you need it.
...
Your note lands in the output directory as `{date}_{seq}_{slug}.md`, ready for Obsidian (or other .md reader).
